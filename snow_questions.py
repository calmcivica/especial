...
database = 'esnowflake'  
username = 'esnowflake'  
password = 'TkUPu2xHB9a$'  


connection_string = f'DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};DATABASE={database};UID={username};PWD={password};timeout=60'
    ...
    if st.session_state.get('exam_mode', 0) == 0:
        ...
        #exam_mode = st.session_state['exam_mode']
            ...
            with st.container():
                ...
                with settings:
                    ...
                    # Duración del examen
                    exam_duration = st.slider('Tiempo de Examen (minutos)', min_value=5, max_value=120, value=60)
        ...
        """
                ...
                with st.container():
                    ...
                    if num_questions == 0:
                        st.warning('Tus filtros u opciones resultan en una cantidad de 0 preguntas')
                    else:
                        with boton:
...
"""     
        ...
        tiempo_invertido_V = conn.cursor().execute(f"select DATEDIFF(SECOND, start_time, end_time) from esnowflake.esnowflake_DEV.FACT_EXAMS where id_exam = {exam_id}").fetchall()
        ...
        preguntas_no_respondidas = len(filtered_answers) - preguntas_acertadas - preguntas_falladas
       ...
       #        Crear un gráfico de donut
        ...
        with volver:
            st.button('Volver al inicio',use_container_width=1,on_click = aux_exam , args = ('Inicio',None,None,))
...
def practicar():
    ...
    with st.expander('¿Como podría usar esta sección? 🤔'):
        st.markdown("""
                🤓 **¡Parece mentira, pero sí, esto es para estudiar Snowflake, no el teórico de la autoescuela!** 🚗💨 A primera vista, podrías pensar que te has equivocado de clase, pero no, estás exactamente donde necesitas estar para dominar Snowflake como un pro.
...
PD: Por si todo esto no fuese suficiente, cada 100 preguntas respondidas tendreís la oportunidad/obligación de decirle a Parra alguna faltada por el grupo. ¿Qué más se puede pedir?
                """)
    
    Filtros,Preguntas = st.columns([1,3],gap = "large")
    with Filtros:
        st.subheader("Filtros")
        values = st.slider('Seleccione rango de preguntas en el que practicar',0, len(datos), (0, len(datos)),step=1,)

        secciones = st.multiselect('¿ Que secciones quieren tocar ?',["Todas","Snowflake Cloud Data Platform Features and Architecture","Account Access and Security","Performance Concepts","Data Loading and Unloading","Data Transformations","Data Protection and Data Sharing"])
        ...
        consulta_preguntas_hechas = f"""SELECT 
        STRING_AGG(CAST(question_id AS NVARCHAR(10)), ',') WITHIN GROUP (ORDER BY question_id) AS Hechas, 
        STRING_AGG(CASE WHEN type = 'Examen' AND is_correct = 0 THEN CAST(question_id AS NVARCHAR(10)) ELSE NULL END, ',') WITHIN GROUP (ORDER BY question_id) AS Examen_falsas, 
        STRING_AGG(CASE WHEN type = 'practicar' AND is_correct = 0 THEN CAST(question_id AS NVARCHAR(10)) ELSE NULL END, ',') WITHIN GROUP (ORDER BY question_id) AS Practicar_falsas 
    ...
    with Preguntas:
        if question_set:
            setexam(question_set, datos, "practicar", conn, user)
        else:
...
def progreso():
    ...
    if user == None:
        ...
        if len(df) == 0:
            ...
            df_resumen = df.groupby('Fecha').size().reset_index(name='Número de Preguntas')
            racha_actual = 0
            fecha_anterior = None
            ...
            # Crear DataFrame de referencia para todas las secciones
            df_secciones_referencia = pd.DataFrame(df_preguntas_totales['question_area'].unique(), columns=['question_area'])
            # Contar el total de preguntas por sección
            total_preguntas_por_seccion = df_preguntas_totales['question_area'].value_counts()
            ...
            metrics_final = df_secciones_referencia.merge(metrics_por_seccion, on='question_area', how='left')
            ...
            for index, row in metrics_final.iterrows():
                labels = ['Preguntas Correctas', 'Preguntas Incorrectas', 'Preguntas No Vistas']
                values = [row['Preguntas Correctas'], row['Preguntas Incorrectas'], row['Preguntas No Vistas']]
            ...
            with secciones:
                ...
                for fecha in reversed(df_resumen['Fecha']):
                    if fecha_anterior is None or fecha_anterior - pd.Timedelta(days=1) == fecha:
                        racha_actual += 1
                ...
                with timeline:
                    ...
                    fig = px.bar(df_resumen, x='Fecha', y='Número de Preguntas', title="Preguntas Respondidas por Día")
            ...
            with exams:
                ...
                columns = ['id_exam', 'start_time', 'duration_minutes', 'number_of_questions', 'number_of_correct_questions', 'number_of_failed_questions']
                ...
                df['number_of_correct_questions'] = df['number_of_correct_questions'].fillna(0)
                df['number_of_failed_questions'] = df['number_of_failed_questions'].fillna(0)

                # Colores para las categorías
                colores = ['green', 'red', 'blue']

                # Tamaño del gráfico
                ancho, alto = 350, 350  # Puedes ajustar estos valores según tus necesidades
                if len(df) == 0:
                ...
                for i, examen in df.iterrows():
                    ...
                    if examen['number_of_questions'] == 0:
                        ...
                        score = examen['number_of_correct_questions'] / examen['number_of_questions']
                    examen['start_time'] = pd.to_datetime(examen['start_time'])
                    timestamp = examen['start_time'].strftime('%Y-%m-%d %H:%M') if pd.notnull(examen['start_time']) else 'Desconocido'
                    ...
                    with st.expander(mensaje):
                        ...
                        with met:
                            ...
                            st.metric("Tiempo Total", f"{examen['duration_minutes']} minutos")
                            if score >= 0.75:
                                st.success("APROBADO")
                            else:
                                st.error("SUSPENSO")
...
if 'current_page' not in st.session_state:
    st.session_state['current_page'] = "Comienzo"

if st.session_state['current_page'] == "Comienzo":
    Comienzo()
